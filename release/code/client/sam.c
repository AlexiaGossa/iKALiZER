...
void CMD_ikalizerGetValue ( void )
{
    ...
    {
        if ( (stricmp(pszName,ikaCvarEntry[i].szCvarNameOld)==0) ||
             (stricmp(pszName,ikaCvarEntry[i].szCvarNameNew)==0) ||
             (stricmp(pszName,ikaCvarEntry[i].szCvarNameNewBisA)==0) ||
             (stricmp(pszName,ikaCvarEntry[i].szCvarNameNewBisB)==0) )
        {
            lFoundEntry = i;
            break;
        }
...
void S_SAM_SoundInfo ( void )
{
    ...
    Com_Printf( "ioUrbanTerror with IKALIZER audio engine : \n" );
    Com_Printf( "  Quake3 version : "Q3_VERSION"\n" );
    Com_Printf( "  ioUrT IKALIZER mod : "__TIMESTAMP__"\n" );
...
#######################################
*/
void S_SAM_RawSamples ( int iSamples, int iSampleRate, int iWidth, int iChannelCount, const byte * pbData, float f32VolumeMul )
{
    pSAM_STREAM_AddData ( 
        iSampleRate,
        iSamples,
...
void S_SAM_StartSound ( vec3_t vec3Origin, int iEntityNumber, int iEntChannel, sfxHandle_t sfxHandle )
{
	...
	psamVoiceSPS->fMasterLevel                  = -6.0F; // fMasterLevelSFX;
...
void S_SAM_StartBackgroundTrack ( const char * pszIntroFileName, const char * pszLoopFileName )
{
    ...
    {
        if (stricmp(pszIntroFileName,pszLoopFileName)==0)
...
void * IKA_QualitySound_Load ( char * pszFileName, snd_info_t * pInfo, float * pfReEqLevel )
{
