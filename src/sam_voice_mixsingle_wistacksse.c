...
void SAM_VOICE_MixSingleGranule_WithStackSSE ( SAM_VOICE * psamVoice, SAM_SFX * psamSFX, float fGlobalGain, DWORD dwLoopCount, float * pfSoftwareBuffer, DWORD dwSoftwareBufferChannelCount, DWORD dwParam )
{
    float       fGlobalGain16;
    ...
    if ((dwParam)&&(render_dwEntryCount>dwParam)) render_dwEntryCount = dwParam;
    ...
    __asm {
            ...
            _loop_render:
                mov         eax, ecx
                mov         ebx, ecx
                add         eax, [esi+edx+  SAM_RENDER254.dwDelayIndex]    //SAM_wi512_dwRender_DelayIndex_LUT+[esi+edx]
                add         ebx, [esi+edx+4+SAM_RENDER254.dwDelayIndex]    //SAM_wi512_dwRender_DelayIndex_LUT+[esi+edx+4]
                and         eax, sam_VOICEBUFFERSTACK_MASK
                and         ebx, sam_VOICEBUFFERSTACK_MASK
                
                movss       xmm0, [esi+edx+  SAM_RENDER254.fDelayGain]//SAM_wi512_fRender_DelayGain_LUT+[esi+edx]
                movss       xmm1, [esi+edx+4+SAM_RENDER254.fDelayGain]//SAM_wi512_fRender_DelayGain_LUT+[esi+edx+4]
