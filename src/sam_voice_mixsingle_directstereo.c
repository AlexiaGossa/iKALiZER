...
/*
    ...
    Special mixing routine...
    
    Output #0 += SFX(mono) * fLevelGainL
    Output #1 += SFX(mono) * fLevelGainR
    
    fLevelGainL/R could be a negative or a positive value.
    

*/

sam_ALIGN   float   _SAM_VOICE_MSG_2ch_fLevelGainLR[4] = { 0, 0, 0, 0 };
...
void SAM_VOICE_MixSingle_DirectStereo ( SAM_VOICE * psamVoice, SAM_SFX * psamSFX, float fGlobalGain, DWORD dwLoopCount, float * pfSoftwareBuffer, DWORD dwSoftwareBufferChannelCount )
{
    ...
    {
        SAM_VOICE_MixSingle_DirectStereo_noresamp (
            psamVoice, psamSFX, fGlobalGain, dwLoopCount, pfSoftwareBuffer, dwSoftwareBufferChannelCount );
        return;
    }
    ...
    _SAM_VOICE_MSG_2ch_fLevelGainLR[0]          = psamVoice->fLevelGainL;
    _SAM_VOICE_MSG_2ch_fLevelGainLR[1]          = psamVoice->fLevelGainR;
    ...
    __asm {
            ...
            _no_new_data:
                ...
                */
                ...
                //movaps      xmm2, [edx]                               //Charge la sortie #0-#3 - FL/FR/RL/RR
                
                movlps      xmm2, [edx]                                 //Charge la sortie #1 & #0 (ou FR & FL)
                
                subss       xmm0, xmm4
                mulss       xmm0, fFX_IIR_LowPassRatio
                addss       xmm4, xmm0
                //movss       xmm3, [edx+4]                             //Charge la sortie #1 - Right
                //movaps      xmm3, [edx+16]                            //Charge la sortie #4-#7
                
                movaps      xmm3, _SAM_VOICE_MSG_2ch_fLevelGainLR       //Charge le gain à appliquer
                ...
                */
                ...
                add         edx, dwSoftwareBufferChannelCount4
...
void SAM_VOICE_MixSingle_DirectStereo_noresamp ( SAM_VOICE * psamVoice, SAM_SFX * psamSFX, float fGlobalGain, DWORD dwLoopCount, float * pfSoftwareBuffer, DWORD dwSoftwareBufferChannelCount )
{
    ...
    __asm {
            ...
            _no_end_of_sound:
                ...
                // ***************************************************
                // Ecriture dans la pile de la nouvelle donnée
                align 16
            _no_end_of_granule:
                movsx       eax, word ptr [esi+eax*2]               //1:1
                mov         dwInPlaySamplePositionPrevious, edi     //1:1
...
void SAM_VOICE_MixSingleGranule_2ch_SSE_old ( SAM_VOICE * psamVoice, SAM_SFX * psamSFX, float fGlobalGain, DWORD dwLoopCount, float * pfSoftwareBuffer, DWORD dwSoftwareBufferChannelCount )
{
    float       fGlobalGain16;
    float       *interpolation_fStackValue;
    ...
    sam_ALIGN   float fLevelGainLR[4];
    ...
    interpolation_fStackValue                   = psamVoice->interpolation_fStackValue;
    lRenderIncrement_F16                        = psamVoice->lRenderIncrement_F16;
    fLevelGainLR[0]                             = psamVoice->fLevelGainL;
    fLevelGainLR[1]                             = psamVoice->fLevelGainR;
    fLevelGainLR[2]                             = 0;
    fLevelGainLR[3]                             = 0;
    ...
    __asm {
            ...
            _no_new_data:
                ...
                */
                ...
                movaps      xmm3, fLevelGainLR                          //Charge le gain à appliquer
                    
                
                movss       fFX_IIR_LowPassValue, xmm4
                
                //*******************************************************
                //Applique le gain L/R
                unpcklps    xmm4, xmm4                  // - | - | s | s
                mulps       xmm4, xmm3
                addps       xmm2, xmm4
