...
#include "sam_data.h"
#include "sam_voice.h"
#include "sam_voice_mixsingle_loadvaluetostack.h"
...
extern  BYTE        XD4_bShiftRight[4];// = { 4, 11, 18, 25 };
...
extern  SAM_GRANULESDATA samGranulesData;
        ...
        //sam_ALIGN   DWORD       sam_voice_mixsingle_dwZeroPS[4]         = { 0x00000000, 0x00000000, 0x00000000, 0x00000000 };
        //sam_ALIGN   DWORD       sam_voice_mixsingle_dwTempPS[4]         = { 0, 0, 0, 0 };
        //sam_ALIGN   DWORD       sam_voice_mixsingle_dwOutputCount[2];

void SAM_VOICE_MixSingleGranule_WithStackSSE_DynamicDelayLines ( SAM_VOICE * psamVoice, SAM_SFX * psamSFX, float fGlobalGain, DWORD dwLoopCount, float * pfSoftwareBuffer, DWORD dwSoftwareBufferChannelCount, DWORD dwMaxDynamicDelayLines )
{
    float       fGlobalGain16;
    ...
    //float       *pfInterpolationFIR_1024_16;
    ...
    dwMaxDynamicDelayLines                      = dwMaxDynamicDelayLines>>1;
    render_dwEntryCount                         = psamVoice->psamRender255->dwEntryCount>>1;
    if (render_dwEntryCount>dwMaxDynamicDelayLines) render_dwEntryCount = dwMaxDynamicDelayLines;
    render_dwEntryCountShl3                     = render_dwEntryCount<<3;
    
    pRender                                     = &samData.psamRender255Table[ ((psamVoice->lRenderPositionCurrent_F16)>>16) ];
    ...
    __asm {
            ...
            _no_new_data:
                ...
                */
                shr         edx, 8                    
                mov         ecx, stack_dwBufferStackPosition
                
                and         edx, 0xFFC0
                dec         ecx
                
                mov         edi, stack_fBufferStackValue
                and         ecx, sam_VOICEBUFFERSTACK_MASK      //IIR+Stack
                ...
                movss       [edi+ecx*4], xmm4                   //edi+ecx*4 = BufferStackValue + stack_dwBufferStackPosition
                movss       fFX_IIR_LowPassValue, xmm4
                //Attention, ecx n'a pas été écrit dans stack_dwBufferStackPosition !
                ...
                */
                ...
                xor         edx, edx
                add         esi, ebx
                mov         stack_dwBufferStackPosition, ecx
                sub         edx, ebx
                mov         edi, stack_fBufferStackValue
                
                //    edx = décompteur (par deux)
                //    ecx = stack_dwBufferStackPosition
                //    esi = offset des 2 LUT (Delay et Gain)
                //    edi = offset de la pile "stack_fBufferStackValue"
            _loop_render:
                mov         eax, ecx
                mov         ebx, ecx
                add         eax, [esi+edx+  SAM_RENDER255.dwDelayIndex]    //SAM_wi512_dwRender_DelayIndex_LUT+[esi+edx]
                add         ebx, [esi+edx+4+SAM_RENDER255.dwDelayIndex]    //SAM_wi512_dwRender_DelayIndex_LUT+[esi+edx+4]
                and         eax, sam_VOICEBUFFERSTACK_MASK
                and         ebx, sam_VOICEBUFFERSTACK_MASK
                
                movss       xmm0, [esi+edx+  SAM_RENDER255.fDelayGain]//SAM_wi512_fRender_DelayGain_LUT+[esi+edx]
                movss       xmm1, [esi+edx+4+SAM_RENDER255.fDelayGain]//SAM_wi512_fRender_DelayGain_LUT+[esi+edx+4]
                
                mulss       xmm0, [edi+eax*4]
                mulss       xmm1, [edi+ebx*4]
                ...
                add         edx, samData.psamRender255Table                 // Interpolation du Render 6/7
